...
def call_wordle_api(guess: str, size: int, seed: int = None):
    """
    Calls the Random Wordle game API to get feedback on a guess.
    """
...
def play_game(word_size: int, seed: int = None):
    """
    Main logic for the Wordle AI player.
    """
    if not 3 <= word_size <= 8:
        print("Error: Word size must be between 3 and 8.")
        return
    ...
    while True:
        ...
        feedback = call_wordle_api(current_guess, word_size, seed)
        if not feedback:
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="An AI-powered CLI to solve the random Wordle puzzle from https://wordle.votee.dev/random.",
        epilog="Example: python guess_2.py --size 6 --seed 12345"
    )
    parser.add_argument(
        "--size", 
        type=int, 
        default=5,
        help="The length of the word to be guessed (default: 5)."
    )
    parser.add_argument(
        "--seed",
        type=int,
        default=None,
        help="A seed for reproducible random words."
    )
    args = parser.parse_args()
    
    play_game(args.size, args.seed)
